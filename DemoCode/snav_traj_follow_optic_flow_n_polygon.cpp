...
*/
  std::vector<Position> positions;
  int point_count = 72;
  float agle_per = 2*M_PI/point_count;
  
  int radius = 1;

  float height_array[5] = {1.5,1,0.5,1,1.5};
  const float yaw_v_k = (M_PI)/(radius*point_count*sin(M_PI/point_count)*radius);
  for(int j=0; j<5; j++){
      for(int k=0;k<=point_count;k++){
         Position pos; 
         pos.x = (1-cos(agle_per*k))*radius;
         pos.y = -sin(agle_per*k)*radius;
         pos.z = height_array[j];
         pos.yaw = agle_per*k;
  ...
  while (mission_in_progress)
  {
    ...
    else
    {
      ...
      else if(state == MissionState::TRAJECTORY_FOLLOW)
      {
        ...
        float yaw_vel_target = yaw_v_k*vel_target;
        ...
        float command_dir_y;
        float command_dir_z;
        float command_dir_yaw;

        float vel_x_target;
        float vel_y_target;
        float vel_z_target;
        float vel_yaw_target;
        ...
        command_dir_yaw = positions[current_position].yaw - (yaw_des-yaw_est_startup);

        float command_mag = sqrt(command_dir_x*command_dir_x +
                                 command_dir_y*command_dir_y +
                                 command_dir_z*command_dir_z);

        if(current_position >= 0.9*(positions.size()) )
        {
        ...
        float yaw_target = atan2(command_dir_y,command_dir_x);
        //yaw_target = positions[current_position].yaw
        ...
        vel_yaw_target = yaw_target/command_dir_yaw*yaw_vel_target;
        ...
        int current_position_div = current_position%point_count;
        if(current_position_div <= 1){
             vel_yaw_des_sent = 0;
        }else if(current_position_div >= 0.9*positions.size()){
             vel_yaw_des_sent = 0;
        }else{
             vel_yaw_des_sent = yaw_vel_target;//fabs(atan2(vel_y_des_sent,vel_x_des_sent));
        }
        float cur_yaw = M_PI*2*current_position_div/point_count;
        if (cur_yaw > M_PI) cur_yaw = cur_yaw - 2*M_PI;
        else if (cur_yaw < -M_PI) cur_yaw = cur_yaw + 2*M_PI;
