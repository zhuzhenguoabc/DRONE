...
*/
  ...
  int radius = 1;
  static float yaw_v_k = (M_PI)/(radius*point_count*sin(M_PI/point_count)*radius);
  for(int k=0;k<=point_count;k++){
    ...
    for(int j=0;j<=point_count;j++){
     Position pos; 
     pos.x = (1-cos(agle_per*j))*radius;
     pos.y = -sin(agle_per*j)*radius;
     pos.z = 2;
     pos.yaw = agle_per*j;
     positions.push_back(pos);
  }
  ...
  while (mission_in_progress)
  {
    ...
    else
    {
      ...
      else if(state == MissionState::TRAJECTORY_FOLLOW)
      {
        ...
        if(current_position <= 1 || (current_position >= point_count && current_position <= point_count+1)){
             vel_yaw_des_sent = 0;
        }else if((current_position >= 0.9*positions.size() && current_position < current_position)  || ((current_position - current_position) >= 0.9*(positions.size()))){
             vel_yaw_des_sent = 0;
        }else{
