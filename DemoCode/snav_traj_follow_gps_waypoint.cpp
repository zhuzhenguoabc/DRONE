...
/*  printf("distance:%f\n", CalcDistance(22.5278256f,113.9334979f,22.5278321f,113.9336830f));
 ...
 printf("distancex:%f\n", CalcAxisDistance((float)1139334979/1e7,(float)1139336830/1e7));
  ...
  if(argc >2)
  {
	posDetination.latitude = atoi(argv[1]);
	posDetination.longitude = atoi(argv[2]);
	printf("posDetination gps latitude,longitude:%d,%d \n",
		posDetination.latitude,posDetination.longitude);
  }
*/
  // Begin mission loop
...
//        float yaw_target;
        ...
        command_diff_x = CalcAxisDistance((float)posDetination.longitude/1e7 ,
			(float)posCurrent.longitude/1e7);

        command_diff_y = CalcAxisDistance((float)posDetination.latitude/1e7 ,
			(float)posCurrent.latitude/1e7);

		distance_to_dest =CalcDistance(
			(float)posDetination.latitude/1e7,
			(float)posDetination.longitude/1e7,
			(float)posCurrent.latitude/1e7,
			(float)posCurrent.longitude/1e7);
		...
		printf("command_mag :%f",command_mag);
      ...
      else if (state == MissionState::LOITER )
      {
        if (props_state == SN_PROPS_STATE_SPINNING)
        {
		  ...
		  printf("LOITER, after gps data:%s\n",buf);
		  ...
		  {
			posDetination.latitude = atoi(gpsparams[2].c_str());
			posDetination.longitude = atoi(gpsparams[1].c_str());
			...
			printf("LOITER posDetination gps latitude,longitude,destyaw,curyaw:%d,%d,%f,%f\n",
				posDetination.latitude,posDetination.longitude,destyaw,yaw_est);

			if(posDetination.latitude !=0 && posDetination.longitude !=0
