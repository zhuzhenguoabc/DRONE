				...
				while(!b_recv_data_result)
				{
					...
					if (i_recv_data_length > 0)
					{
...

						{
							...
							printf("[%d]:circle_receive prodcons fd_socket=%d, pro_tcp_receive=%s, time_now=%lf\n\n",
							...
							&& (strcmp(pro_tcp_receive.data, SNAV_TASK_GET_INFO) != 0))
						{
							...
							&& (errno == EINTR
								...
								|| errno == EAGAIN))
					{
						printf("[%d]circle_receive recv return i_recv_data_length=%d, errno=%d\n", count,i_recv_data_length, errno);
						fflush(stdout);
...
void* circle_send(void* arg)
{
	...
	while(true)
	{
		...
		if (bLocalSocketFlag)
		{
			printf("[%d]:circle_send quit from circle_receive socket closed!\n\n",count);
			fflush(stdout);
...

		{
			...
			gettimeofday(&time_val, NULL);
			double time_now = time_val.tv_sec + time_val.tv_usec * 1e-6;
			printf("[%d]:circle_send check pro_tcp_send.bflag time_now=%lf\n", count, time_now);
		}
...
void* snav_handler(void* arg)
{
	...
	while (true)
	{
...

		{
			...
			gettimeofday(&time_val, NULL);
			double time_now = time_val.tv_sec + time_val.tv_usec * 1e-6;
			printf("[%d]:snav_handler thread before udp recvfrom time_now=%lf\n", loop_counter, time_now);
		}
			...
			*/
				...
				&& (gpsparams_udp[0].compare(SNAV_CMD_CONROL)==0))
			{
				...
				else
				{
					...
					if (bLocalTcpFlag)
					{
						...
						printf("[%d]:prepare pro_tcp_send to send back to client\n",loop_counter);
			...
			else if (state == MissionState::LANDING)
			{
				...
				if (props_state == SN_PROPS_STATE_SPINNING)
				{
					...
					printf("[%d] [Landing distance_to_ground]: [%f]\n",
			...
			else if (state == MissionState::LOITER )
			{
				if (props_state == SN_PROPS_STATE_SPINNING)
				{
					...
					else if(cur_body.have_face && face_body_follow_switch) //cuiyc add face detect begin
					{
						float face_offset ;
			...
			printf("[%d] Current Sonar range: [%f]\n", loop_counter,snav_data->sonar_0_raw.range);
	...
	printf("out from the snav_handler thread\n\n");
...
void* getVideoFaceFollowParam(void*)
{
	...
	printf("getVideoFaceFollowParam start");
...
int main(int argc, char* argv[])
{
	...
	while(!handler_flag)
	{
		...
		printf("create snav_handler thread\n");
		fflush(stdout);
	...
	while(!face_body_follow_flag)
	{
		...
		printf("create face_body_follow thread\n");
		fflush(stdout);
	...
	*/
    ...
    while (1)
    {
		...
		printf("tcp_receive_thread_flag=%d, tcp_send_thread_flag=%d\n", tcp_receive_thread_flag, tcp_send_thread_flag);
        ...
        printf("get connection from %s, client_sockfd=%d\n\n",ip_address,client_sockfd);
